...
function applyPointEdgeCollisionFilter(dt) {
    ...
    // Populate grid once for ALL collisions
    grid2D.populateGrid(blobs, environment);

    // DEBUG: Count objects in grid
    let totalEdgesInGrid = 0;
    let environmentEdgesInGrid = 0;
    let blobEdgesInGrid = 0;
    
    for(let i = 0; i < grid2D.cells.length; i++) {
        for (let obj of grid2D.cells[i].objects) {
            if (obj instanceof Edge) {
                totalEdgesInGrid++;
                if (obj.q.blob == null && obj.r.blob == null) {
                    environmentEdgesInGrid++;
                } else {
                    blobEdgesInGrid++;
                }
            }
    ...
    console.log(`Grid contains: ${totalEdgesInGrid} total edges, ${environmentEdgesInGrid} environment edges, ${blobEdgesInGrid} blob edges`);

    let collisionCount = 0;
    let pegCollisionCount = 0;
    let spikeCollisionCount = 0;
    let blobBlobCollisionCount = 0;

    for (let iter = 0; iter < MAX_IMPULSE_ITERATIONS; iter++) {
        // === SINGLE PASS FOR ALL COLLISIONS: BLOB-vs-ENVIRONMENT + BLOB-vs-BLOB ===
        for (let blob of blobs) {
            for (let p of blob.BP) {
                ...
                for (let obj of nearbyObjects) {
                    if (obj instanceof Edge) {
                        ...
                        if (edgeType === "peg/wall") {
                            console.log(`PEG COLLISION: particle at (${p.p.x.toFixed(3)}, ${p.p.y.toFixed(3)}) with ${edgeType} edge, length: ${obj.length().toFixed(3)}, rigid: ${obj.isRigid()}`);
                        }

                        if (isSpikeEdge(obj)) {
                            processSpikeCollision(p, obj, k_p, k_d_penalty, d0);
                            processSpikeImpulse(p, obj, e, dt);
                        } else {
                            // Regular edges (walls, pegs, other blob edges)
                            processPenalty(p, obj, k_p, k_d_penalty, d0);
                            processImpulse(p, obj, e, dt);
                        }
                    }
                }
            }
    ...
    if (collisionCount > 0) {
        console.log(`Collision Summary - Total: ${collisionCount}, Pegs/Walls: ${pegCollisionCount}, Spikes: ${spikeCollisionCount}, Blob-Blob: ${blobBlobCollisionCount}`);
    }
}
...
function isSpikeEdge(edge) {
    // Spikes are very short, rigid environment edges
    return edge.isRigid() && 
           edge.length() < 0.02 && // Spikes are very short (adjust this threshold as needed)
           edge.q.blob == null && edge.r.blob == null; // Environment edges
}

function processSpikeCollision(p, spike, k_p, k_d_penalty, d0) {
    if (p === spike.q || p === spike.r) return;
    
    let result = closestPointOnSegment(p.p, spike.q.p, spike.r.p);
    let c = result.point;
    let t = result.t;
    let v_pc = vsub(p.p, c);
    let d = v_pc.mag();
    ...
    if (penetration > 0 && d < d0 * 0.95) {
        ...
        // 🔥 SPIKES APPLY STRONGER, SHARPER FORCES
        let spikeForceMultiplier = 4.0; // Spikes are more penetrating
        let f_spring = k_p * penetration * spikeForceMultiplier;
        
        let v_rel = p.v;
        let v_n = v_rel.dot(n);
        let f_damp = -k_d_penalty * v_n;
        ...
        // 🎯 SPIKE-SPECIFIC DEFORMATION EFFECTS
        
        // 1. Apply strong tangential "tearing" force
        let tangent = vec2(-n.y, n.x);
        let v_t = v_rel.dot(tangent);
        let tearForce = vmult(tangent, -1.2 * k_d_penalty * v_t);
        p.f.add(tearForce);
        
        // 2. Strong position correction for deep penetration
        if (penetration > d0 * 0.3) {
            let push = vmult(n, 0.4 * penetration);
            p.p.add(push);
        }
        
        // 3. Mark blob as "hit" for visual effects
        if (p.blob && penetration > d0 * 0.5) {
            p.blob.hit = true;
            p.blob.hitTimer = 30; // frames of visual effect
            
            // Add velocity jitter for more dramatic effect
            p.v.add(vec2(random(-0.01, 0.01), random(-0.01, 0.01)));
        }
    }
}

function processSpikeImpulse(p, spike, e, dt) {
    // Similar to processImpulse but with spike-specific behavior
    if (p === spike.q || p === spike.r) return;
    ...
    const invMass_p = p.invMass();
    const invMass_eff = invMass_p; // Spikes are rigid (infinite mass)

    // Spikes have lower restitution (more "sticky", less bouncy)
    let spikeRestitution = e * 0.3; // 70% less bouncy than regular edges
    let v_restitution = -spikeRestitution * v_n;

    // Stronger position correction for spikes
    const beta = 0.15;
    const slop = 0.0001;
    let v_correction = 0;
    if (dt > 1e-9)
    ...
    // Add some random velocity for more dramatic spike effect
    if (penetration > PARTICLE_RADIUS * 0.5) {
        p.v.add(vec2(random(-0.005, 0.005), random(-0.005, 0.005)));
    }
}

function identifyEdgeType(edge) {
    if (edge.q.blob != null || edge.r.blob != null) {
        return "blob-edge";
    }
    ...
    // Pegs are typically shorter than walls but longer than spikes
    const length = edge.length();
    if (length < 0.03) {
        return "spike"; // Very short = spike
    } else if (length < 0.1) {
        return "peg";   // Medium length = peg
    } else {
        return "wall";  // Long = wall
    }
}